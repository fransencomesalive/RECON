...
export default function ProcessingPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const windP = post('/api/enrich/wind', { id: analyzeId })
        .then(data => {
          if (cancelled) return
          if (data.error) { errorsRef.current.wind = data.error; setStatus('wind', 'error') }
          else { windRef.current = data; setStatus('wind', 'done') }
        })
